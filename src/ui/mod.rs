...
pub fn accent() -> Color {
    Color::Rgb(
        ACCENT_R.with(|c| c.get()),
        ACCENT_G.with(|c| c.get()),
        ACCENT_B.with(|c| c.get()),
    )
}
...
pub fn bar_progress() -> Color {
    Color::Rgb(
        BAR_PROG_R.with(|c| c.get()),
        BAR_PROG_G.with(|c| c.get()),
        BAR_PROG_B.with(|c| c.get()),
    )
}
...
pub fn bar_complete() -> Color {
    Color::Rgb(
        BAR_DONE_R.with(|c| c.get()),
        BAR_DONE_G.with(|c| c.get()),
        BAR_DONE_B.with(|c| c.get()),
    )
}
...
fn draw_header(f: &mut Frame, app: &App, area: Rect) {
    // Fill header bg
    f.render_widget(Block::default().style(Style::default().bg(C_BG2)), area);
...
fn draw_body(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let cols =
        Layout::horizontal([Constraint::Percentage(26), Constraint::Percentage(74)]).split(area);
...
fn draw_right(f: &mut Frame, app: &mut App, area: Rect) {
    let rows = Layout::vertical([Constraint::Percentage(55), Constraint::Min(0)]).split(area);
...
pub fn trunc(s: &str, max: usize) -> String {
    ...
    if c.len() <= max {
        ...
        c[..max.saturating_sub(1)].iter().collect::<String>() + "…"
    }
}
