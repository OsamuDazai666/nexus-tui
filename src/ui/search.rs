...
use crate::ui::{trunc, C_BG, C_BG2, C_BG3, C_BORDER_F, C_DIM, C_PANEL, C_SCORE, C_TEXT};
use ratatui::{
    layout::Rect,
...
pub fn draw_results(f: &mut Frame, app: &App, area: Rect) {
        ...
        .take(max_visible)
    {
        ...
        if item_area.y + item_area.height > inner.y + inner.height {
            break;
        }
        ...
        let content = Paragraph::new(vec![
            Line::from(vec![
                Span::styled(
                    trunc(item.title(), (item_area.width as usize).saturating_sub(15)),
                    if sel {
                        Style::default()
                            .fg(crate::ui::accent())
                            .add_modifier(Modifier::BOLD)
